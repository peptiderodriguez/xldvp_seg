...
def _stratified_sample(uids, transect_records, n_target, seed=42):
    ...
    if remainder > 0:
        sorted_by_size = sorted(path_to_uids.keys(),
                                key=lambda p: len(path_to_uids[p]), reverse=True)
...
def main():
    ...
    parser.add_argument('--transect-csv', required=True,
    ...
    parser.add_argument('--no-empty-wells', action='store_true',
                        help='Disable QC empty well insertion')
    parser.add_argument('--zonation-scores-csv', default=None,
    ...
    if path_filter_set is not None:
        ...
        transect_records = {
            uid: rec for uid, rec in transect_records.items()
            if rec['path_id'] in path_filter_set
        }
        logger.info(f"Path filter ({', '.join(sorted(path_filter_set))}): "
    ...
    if frac_lo > 0.0 or frac_hi < 1.0:
        ...
        logger.info(f"Frac range [{frac_lo:.3f}, {frac_hi:.3f}]: "
                    f"{before:,} -> {len(transect_records):,} cells")
    ...
    for det in detections:
        ...
        if uid in target_uids:
            uid_to_det[uid] = det
    logger.info(f"Matched {len(uid_to_det):,} / {len(target_uids):,} transect UIDs "
                f"in detections JSON")

    missing = target_uids - uid_to_det.keys()
    if missing:
        logger.warning(f"  {len(missing):,} transect UIDs not found in detections JSON "
    ...
    if args.score_threshold > 0.0:
        ...
        uid_to_det = {
            uid: det for uid, det in uid_to_det.items()
            if _get_score(det, score_key) >= args.score_threshold
        }
        logger.info(f"Score filter (>= {args.score_threshold} on '{score_key}'): "
    ...
    if args.max_cells is not None and len(selected_uids) > args.max_cells:
        logger.info(f"Subsampling {len(selected_uids):,} -> {args.max_cells} cells "
    ...
    selected_uids.sort(key=lambda u: (
    ...
    if not args.no_empty_wells and n_selected > 0:
        plate_well_pairs, empty_positions = insert_empty_wells(
            plate_well_pairs, n_selected,
            empty_pct=args.empty_pct,
            seed=args.seed,
        )
        logger.info(f"Inserted {len(empty_positions)} QC empty wells "
                    f"({args.empty_pct:.0f}% of {n_selected})")

    n_plates = plate_well_pairs[-1][0] if plate_well_pairs else 1
    logger.info(f"Well assignment: {n_selected} cells + {len(empty_positions)} empty "
    ...
    logger.info(f"Wrote filtered detections: {filtered_json_path} "
                f"({len(output_detections):,} cells)")
    ...
    csv_fieldnames = ['cell_uid', 'path_id', 'path_name', 'fractional_pos',
                      'plate', 'well'] + marker_cols
    if global_scores:
    ...
    logger.info(f"  Transect CSV cells:      {n_csv_total:,} (total), "
    ...
    logger.info(f"  Run LMD export with:")
    logger.info(f"    python run_lmd_export.py \\")
    logger.info(f"      --detections {filtered_json_path} \\")
    logger.info(f"      --crosses <crosses.json> \\")
