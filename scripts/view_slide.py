...
def run_classify_markers(det_json: Path, markers: list[dict], czi_path: Path | None,
    ...
    print(f"\n{'=' * 60}")
    print(f"Step 1: Marker classification")
    print(f"{'=' * 60}")
    for m in markers:
        method_desc = {
    ...
    for group in groups:
        ...
        cmd = [
            sys.executable, str(REPO / 'scripts' / 'classify_markers.py'),
            '--detections', str(current_json),
            '--marker-channel', channels,
            '--marker-name', names,
            '--method', method,
...
def run_spatial_analysis(det_json: Path, pixel_size: float, spatial_filter: str | None,
                         output_dir: Path) -> Path:
    """Run spatial_cell_analysis.py --spatial-network. Returns path to enriched JSON."""
    cmd = [
        sys.executable, str(REPO / 'scripts' / 'spatial_cell_analysis.py'),
        '--detections', str(det_json),
        '--output-dir', str(output_dir),
    ...
    print(f"\n{'=' * 60}")
    print(f"Step 2: Spatial network analysis")
    print(f"{'=' * 60}")
    print(f"  Method: Delaunay triangulation → edge pruning → connected components")
    print(f"  Max edge distance: 50 um (cells farther apart are not neighbors)")
    print(f"  Min component size: 3 cells (smaller groups ignored)")
    if spatial_filter:
        print(f"  Cell filter: {spatial_filter}")
        print(f"  (only filtered cells participate in the spatial graph)")
    print()
    result = subprocess.run(cmd)
    if result.returncode != 0:
...
def run_spatial_viewer(det_json: Path, group_field: str, title: str,
                       output_dir: Path) -> Path:
    ...
    cmd = [
        sys.executable, str(REPO / 'scripts' / 'generate_multi_slide_spatial_viewer.py'),
        '--detections', str(det_json),
    ...
    print(f"\n{'=' * 60}")
    print(f"Step 3: Generating interactive spatial viewer")
    print(f"{'=' * 60}")
    print(f"  Coloring cells by: {group_field}")
    print(f"  Viewer features:")
    print(f"    - DBSCAN spatial clustering with auto-eps (KNN knee method)")
    print(f"    - Eps scale slider: adjust cluster tightness in the browser")
    print(f"    - Convex hull outlines around clusters (min 24 cells)")
    print(f"    - Min cells slider for DBSCAN min_samples")
    print(f"    - ROI drawing (circle, rectangle, polygon) with JSON export")
    print(f"    - Per-group legend with show/hide toggle")
    print(f"    - Pan/zoom per slide panel, focus view on double-click")
    print()
    result = subprocess.run(cmd)
    if result.returncode != 0:
...
def serve_directory(directory: Path, port: int):
    ...
    http_proc, tunnel_proc, tunnel_url = start_server_and_tunnel(
        serve_dir, port=port, background=False,
        slide_name=slide_name, cell_type='view_slide',
    )
...
def parse_markers_inline(spec: str) -> list[dict]:
    ...
    for item in spec.split(','):
        ...
        if len(parts) < 2:
            print(f"ERROR: Invalid marker spec '{item}'. Expected 'name:channel[:method]'",
                  file=sys.stderr)
            sys.exit(1)
        markers.append({
            'name': parts[0].strip(),
...
def main():
    ...
    parser.add_argument('run_dir', type=Path,
                        help='Run directory (timestamped, with tiles/ and *_detections.json)')
    ...
    parser.add_argument('--spatial', action='store_true',
                        help='Run spatial clustering (Delaunay + communities)')
    parser.add_argument('--spatial-filter', default=None,
                        help='Filter expression for spatial (e.g. "tdTomato_class==positive")')
    parser.add_argument('--group-field', default='marker_profile',
                        help='Field to color cells by in viewer (default: marker_profile)')
    parser.add_argument('--title', default=None,
                        help='Viewer title (default: slide name)')
    parser.add_argument('--port', type=int, default=None,
                        help='HTTP server port (default: auto-detect free port from 8080)')
    parser.add_argument('--no-serve', action='store_true',
                        help='Generate viewer HTML without serving')
    parser.add_argument('--force', action='store_true',
                        help='Re-run classification even if classified file exists')
    ...
    if spatial:
        if pixel_size is None:
            print("ERROR: --spatial requires pixel_size_um but pipeline_config.json "
                  "not found or missing pixel_size_um.", file=sys.stderr)
