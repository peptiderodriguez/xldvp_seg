...
def run_clustering(args):
    ...
    if args.gate_channel is not None:
        ...
        print(
            f"  Gated on {gate_key} >= p{args.gate_percentile:.0f} ({gate_cutoff:.1f}): "
    ...
    if len(detections) < args.min_cluster_size:
        print(
            f"ERROR: Not enough detections ({len(detections)}) for clustering "
    ...
    if marker_channels is None:
        ...
        if marker_channels is None:
            print(
                "WARNING: No marker channels found in features. "
        ...
        for name, idx in list(marker_channels.items()):
            if idx not in all_channels:
                print(
                    f"  WARNING: Marker '{name}' references ch{idx} which has no "
                    f"features in detections (available: {all_channels})"
                )
            if idx in exclude_channels:
                print(
                    f"  WARNING: Marker '{name}' references ch{idx} which is in "
    ...
    if args.spatial_smooth:
        ...
        if positions is not None and len(positions) == X_scaled.shape[0]:
            ...
            print(
                f"[spatial-smooth] WARNING: Could not extract positions for all cells "
    ...
    if X_scaled.shape[1] > 50:
        ...
        if total_var < pca_target:
            print(
                f"  WARNING: All {X_umap.shape[1]} components kept but only "
                f"{total_var:.1%} variance explained (< {pca_target:.0%} target)"
            )
        else:
            print(
                f"  PCA: {X_scaled.shape[1]} -> {X_umap.shape[1]} dims "
    ...
    if args.methods in ("umap", "both"):
        print(
            f"Running UMAP (n_neighbors={args.n_neighbors}, min_dist={args.min_dist}, n_jobs={n_jobs})..."
        )
        reducer = umap.UMAP(
    ...
    if args.methods in ("tsne", "both"):
        ...
        print(
            f"Running t-SNE (perplexity={args.perplexity}, max_iter={args.tsne_n_iter}, n_jobs={n_jobs})..."
        )
        tsne = TSNE(
    ...
    if args.trajectory:
        try:
            ...
            if "X_draw_graph_fa" in adata.obsm:
                ...
                plt.close(fig)
                print("  Saved: force_directed.png")
            ...
            if has_dpt and umap_embedding is not None:
                ...
                plt.close(fig)
                print("  Saved: pseudotime_umap.png")
            ...
            if has_dpt and umap_embedding is not None:
                try:
                    ...
                    print("  Saved: pseudotime_interactive.html")
                except ImportError:
                    pass
...
def run_subclustering(
    ...
    print(f"\n{'='*60}")
    print(
        f"SUB-CLUSTERING by appearance ({len(sub_feature_names)} features: "
