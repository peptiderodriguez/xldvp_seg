...
def classify_gmm(values: np.ndarray) -> tuple[float, np.ndarray]:
    ...
    if separation < 0.5:
        logger.warning(
            f"GMM components poorly separated (separation={separation:.2f}). "
...
def extract_marker_values(
    ...
    if values.max() == 0:
        ...
        if use_raw:
            logger.warning(
                f"Key '{key}' not found or all zeros — " f"falling back to '{fallback_key}'"
            )
            values = np.array(
...
def classify_single_marker(
    ...
    logger.info(
        f"Classifying marker '{marker_name}' (ch{channel}) with method '{method}' "
    ...
    logger.info(
        f"  Extracted {len(raw_values):,} values, "
    ...
    if intensity_feature == "snr":
        ...
        if normalize_snr is not None:
            ...
            logger.info(
                f"  SNR normalized by reference channel: "
        ...
        logger.info(
            f"  After subtraction: {len(nonzero):,}/{len(values):,} cells with signal, "
            f"range [{nonzero.min():.1f}, {nonzero.max():.1f}], "
            f"median {np.median(nonzero):.1f}"
            if len(nonzero) > 0
            else "  All values zero after subtraction"
        )
    elif bg_subtract:
    ...
    if cv_max is not None:
        ...
        logger.info(
            f"  CV filter: {n_cv_filtered:,} cells with CV > {cv_max:.1f} "
    ...
    logger.info(
        f"  Threshold: {threshold:.2f}" f"{' (on bg-subtracted values)' if bg_subtract else ''}"
    )
    logger.info(
        f"  Positive: {n_positive:,} ({pct:.1f}%)  "
