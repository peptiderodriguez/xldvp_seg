...
def find_vessel_structures(positions, radii_um, min_cells=3):
    ...
    for radius in radii_um:
        ...
        logger.info(
            f"  Radius {radius} um: {len(components)} structures "
...
def infer_vessel_type(
    component_indices,
    all_detections,
    morphology,
    sma_snr_key="SMA_snr",
    cd31_snr_key="CD31_snr",
    sma_class_key="SMA_class",
    cd31_class_key="CD31_class",
):
...
def analyze_vessel_structures(
    detections, positive_indices, positions, radii_um, min_cells, marker_names, snr_keys, class_keys
):
    ...
    """
    logger.info(f"Analyzing {len(positive_indices)} positive cells at " f"radii: {radii_um} um")
...
def enrich_detections(detections, structures, best_radius):
    ...
    logger.info(
        f"Enriched {n_assigned} detections with vessel structure info " f"(radius={best_radius} um)"
    )
    return detections
...
def run_squidpy_analysis(
    detections, positive_indices, positions, output_dir, marker_names, class_keys, snr_keys
):
...
def run_leiden_clustering(detections, positions, output_dir, resolution=0.5):
    ...
    for k in sorted(sample_feat.keys()):
        ...
        elif k.startswith("ch") and k.split("_")[-1] in ("mean_raw", "snr", "std", "max", "median"):
            feature_keys.append(k)
        elif (
            not k.startswith("ch")
            and not k.startswith("sam2")
            and not k.startswith("SMA")
            and not k.startswith("CD31")
            and not k.startswith("marker")
            and not k.startswith("vessel")
        ):
            v = sample_feat[k]
    ...
    sc.pl.umap(
        adata, color="leiden", ax=axes[0], show=False, title=f"UMAP — Leiden (res={resolution})"
    )
    sc.pl.umap(adata, color="marker_profile", ax=axes[1], show=False, title="UMAP — Marker Profile")
    plt.tight_layout()
    plt.savefig(leiden_dir / "umap_leiden.png", dpi=150, bbox_inches="tight")
    plt.close()
    logger.info("  Saved umap_leiden.png")
    ...
    plt.close()
    logger.info("  Saved umap_combined.png")
    ...
    plt.close()
    logger.info("  Saved spatial_leiden.png")

    # Cluster composition
    ct = pd.crosstab(adata.obs["leiden"], adata.obs["marker_profile"], normalize="index")
    ...
    logger.info("  Saved vessel_adata.h5ad")
...
def parse_args():
    ...
    parser.add_argument(
        "--detections",
        required=True,
        help="Path to classified detections JSON " "(from classify_markers.py)",
    )
    parser.add_argument(
        "--marker-field",
        default="marker_profile",
        help="Feature field with combined marker label " "(default: marker_profile)",
    )
    parser.add_argument(
        "--positive-values",
    ...
    parser.add_argument(
        "--min-cells", type=int, default=3, help="Minimum cells per structure (default: 3)"
    )
    parser.add_argument(
        "--output-dir", default=None, help="Output directory (default: same as detections)"
    )
    parser.add_argument(
        "--generate-viewer", action="store_true", help="Generate interactive spatial viewer HTML"
    )
    parser.add_argument("--run-squidpy", action="store_true", help="Run squidpy spatial statistics")
    parser.add_argument(
        "--run-leiden",
        action="store_true",
        help="Run Leiden clustering on full feature space " "(morph + channel SNR + SAM2)",
    )
    parser.add_argument(
        "--leiden-resolution", type=float, default=0.5, help="Leiden resolution (default: 0.5)"
    )
    parser.add_argument(
        "--viewer-group-field",
        default="vessel_type",
...
def main():
    ...
    positive_mask = np.array(
        [d.get("features", {}).get(marker_field, "") in positive_values for d in detections]
    )
    positive_indices = np.where(positive_mask)[0]
    logger.info(
        f"Positive cells: {len(positive_indices):,} / {len(detections):,} "
            ...
            slim.append(
                {
                    ...
                    "features": {k: v for k, v in d.get("features", {}).items() if k in keep_keys},
                }
            )
