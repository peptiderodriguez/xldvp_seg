...
def load_czi_pyramid(
    czi_path, channel=0, scale_factors=(2, 8), flip_horizontal=False, rotate_cw_90=False
):
    ...
    peek = np.squeeze(
        czi.read_mosaic(region=(x0, y0, fw, min(STRIP_H, fh)), scale_factor=1.0, C=channel)
    )
    ...
    for i, row_start in enumerate(range(0, fh, STRIP_H)):
        ...
        strip = np.squeeze(
            czi.read_mosaic(region=(x0, y0 + row_start, fw, h), scale_factor=1.0, C=channel)
        )
...
def load_zarr_pyramid(zarr_path, level_indices=(1, 3), flip_horizontal=False, rotate_cw_90=False):
    ...
    if color_by:
        ...
        for i, (group_name, polys) in enumerate(sorted(groups.items())):
            ...
            lyr = viewer.add_shapes(
                dashes,
                shape_type="path",
                edge_color=color_rgba,
                edge_width=3,
                face_color=[0, 0, 0, 0],
                name=f"{group_name} ({len(polys)})",
                opacity=0.8,
            )
            lyr.mouse_pan = False
...
def load_sampling_overlay(
    ...
    for i, rep in enumerate(replicates):
        ...
        lyr = viewer.add_points(
            np.array(pts),
            name=f"Rep{rep_num} ({well})",
            symbol="disc",
            size=np.array(sizes),
            face_color=np.array([color]),
            edge_color=np.array([color]),
            opacity=0.4,
        )
        lyr.mouse_pan = False
        lyr.mouse_zoom = False
...
def run_single_slide(args, input_path=None, output_path=None):
    ...
    if contours_path and Path(contours_path).exists():
        ...
        try:
            load_contour_overlay(
                ...
                orig_h=contour_orig_h,
                orig_w=contour_orig_w,
                flip_h=flip_h,
                rot90=rotate,
    ...
    cross_layers = []
    for i in range(3):
        dummy = [np.array([[-1e8, -1e8], [-1e8, -1e8 + 1], [-1e8 + 1, -1e8 + 1], [-1e8 + 1, -1e8]])]
        lyr = viewer.add_shapes(
            dummy,
            shape_type="polygon",
            name=CROSS_LABELS[i],
            face_color=CROSS_COLORS[i],
            edge_color=CROSS_COLORS[i],
            edge_width=0,
            opacity=1.0,
        )
        lyr.mouse_pan = False
    ...
    def _(v):
        ...
        for i in reversed(range(3)):
            if positions[i] is not None:
                ...
                cross_layers[i].data = [
                    np.array(
                        [[-1e8, -1e8], [-1e8, -1e8 + 1], [-1e8 + 1, -1e8 + 1], [-1e8 + 1, -1e8]]
                    )
                ]
                active[0] = i
    ...
    def _(v):
        for i in range(3):
            positions[i] = None
            cross_layers[i].data = [
                np.array([[-1e8, -1e8], [-1e8, -1e8 + 1], [-1e8 + 1, -1e8 + 1], [-1e8 + 1, -1e8]])
            ]
        active[0] = 0
...
def main():
    ...
    parser.add_argument("--input", "-i", type=str, default=None, help="CZI or OME-Zarr path")
    parser.add_argument(
        "zarr_path", nargs="?", type=str, default=None, help="[Deprecated] Use -i instead"
    )
    parser.add_argument("--output", "-o", type=str, default=None, help="Output crosses JSON")

    # CZI options
    parser.add_argument("--channel", type=int, default=0, help="CZI channel (default: 0)")
    parser.add_argument(
        "--scale-factor",
        type=int,
        default=2,
        help="CZI base downsampling (default: 2 = 1/2 res). " "Second level is auto 4x coarser.",
    )
    parser.add_argument(
        "--pyramid-levels",
        type=int,
        nargs="+",
        default=None,
        help="CZI downsampling factors (e.g. --pyramid-levels 2 8). " "Overrides --scale-factor.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="1/2",
        help='Zarr pyramid level: "1/2", "1/4", "1/8", or integer ' "(default: 1/2)",
    )

    # Display transforms
    parser.add_argument(
        "--flip-horizontal", action="store_true", help="Mirror horizontally (tissue-down for LMD7)"
    )
    parser.add_argument(
        "--rotate-cw-90",
        action="store_true",
    ...
    # Overlays
    parser.add_argument("--contours", type=str, default=None, help="Contour JSON for overlay")
    parser.add_argument(
        "--slide", type=str, default=None, help="Slide name filter for per-slide contour files"
    )
    parser.add_argument(
        "--color-by",
        type=str,
        default=None,
        help="Color contours by field (e.g. group, classification, " "score_class, tdTomato_class)",
    )
    parser.add_argument(
        "--sampling-results",
        type=str,
    ...
    # Other
    parser.add_argument(
        "--pixel-size", type=float, default=None, help="Pixel size um (auto-detected if not set)"
    )
    parser.add_argument("--fresh", action="store_true", help="Don't auto-load existing crosses")

    # Batch mode
    parser.add_argument("--czi-dir", type=str, default=None, help="Batch: directory of CZIs")
    parser.add_argument("--slides", nargs="+", default=None, help="Batch: slide name prefixes")
    parser.add_argument("--output-dir", type=str, default=None, help="Batch: output directory")

    # Backward compat
    parser.add_argument(
        "--detections", "-d", type=str, default=None, help="[Deprecated] Use --contours"
    )
    parser.add_argument(
        "--load-existing",
        type=str,
