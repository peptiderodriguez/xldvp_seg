...
def main():
    ...
    agg_df = df.groupby(['slide', 'short', 'sex', 'treatment', 'replicate', 'bone'])[feature_cols].median().reset_index()

    # Merge density info
    agg_df = agg_df.merge(
        density_df[['slide', 'bone', 'n_cells', 'tissue_area_mm2', 'density_per_mm2', 'tissue_fraction']],
        on=['slide', 'bone'], how='left'
    )
    ...
    for feat in all_features:
        ...
        if n_out > 0:
            ...
            for idx in agg_df[feat_outliers].index:
                row = agg_df.loc[idx]
                print(f"    {row['short']:8s} {row['bone']:10s} {feat:25s} = {vals.get(idx, '?'):.4g}  (outside [{lo:.4g}, {hi:.4g}])")
    ...
    for bone in ['femur', 'humerus']:
        ...
        comparisons.append((f'Treatment|{bone}', 'treatment', {'GC': 'GC', 'HU': 'HU'}, {'bone': bone}))

    # --- 3. Within-sex comparisons ---
    for sex in ['F', 'M']:
        comparisons.append((f'Treatment|{sex}', 'treatment', {'GC': 'GC', 'HU': 'HU'}, {'sex': sex}))
        comparisons.append((f'Bone|{sex}', 'bone', {'femur': 'femur', 'humerus': 'humerus'}, {'sex': sex}))

    # --- 4. Within-treatment comparisons ---
    for trt in ['GC', 'HU']:
        comparisons.append((f'Sex|{trt}', 'sex', {'F': 'F', 'M': 'M'}, {'treatment': trt}))
        comparisons.append((f'Bone|{trt}', 'bone', {'femur': 'femur', 'humerus': 'humerus'}, {'treatment': trt}))

    # --- 5. Three-way: within sex×bone ---
    for sex in ['F', 'M']:
        for bone in ['femur', 'humerus']:
            comparisons.append((f'Treatment|{sex}_{bone}', 'treatment', {'GC': 'GC', 'HU': 'HU'}, {'sex': sex, 'bone': bone}))

    # --- 6. Three-way: within sex×treatment ---
    for sex in ['F', 'M']:
        for trt in ['GC', 'HU']:
            comparisons.append((f'Bone|{sex}_{trt}', 'bone', {'femur': 'femur', 'humerus': 'humerus'}, {'sex': sex, 'treatment': trt}))

    # --- 7. Three-way: within treatment×bone ---
    for trt in ['GC', 'HU']:
        for bone in ['femur', 'humerus']:
            comparisons.append((f'Sex|{trt}_{bone}', 'sex', {'F': 'F', 'M': 'M'}, {'treatment': trt, 'bone': bone}))
    ...
    stb_groups = [f'{s}_{t}_{b}' for s in ['F','M'] for t in ['GC','HU'] for b in ['femur','humerus']]
    ...
    for sex in ['F', 'M']:
        comparisons.append((f'Ratio_Treatment|{sex}', 'treatment', {'GC': 'GC', 'HU': 'HU'}, {'sex': sex}))
    ...
    for comp_name, group_col, group_vals, filters in comparisons:
        ...
        for feat in test_features:
            ...
            results.append({
                ...
                'cohens_d': d,
                'abs_cohens_d': abs(d),
                'direction': f"{g1_label}>{g2_label}" if np.median(v1) > np.median(v2) else f"{g2_label}>{g1_label}",
            })
    ...
    for kw_name, group_col, filters in kw_tests:
        ...
        for feat in all_features:
            ...
            eta_sq = (stat - len(group_values) + 1) / (n_total - len(group_values)) if n_total > len(group_values) else 0
    ...
    for int_name, factor_a, factor_b in interaction_tests:
        ...
        for feat in all_features:
            ...
            multigroup_results.append({
                ...
                'n_groups': len(working_df[factor_a].unique()) * len(working_df[factor_b].unique()),
    ...
    save_cols = ['comparison', 'feature', 'test_type', 'is_ratio', 'group_var',
                 'g1_label', 'g2_label', 'g1_median', 'g2_median', 'g1_n', 'g2_n',
                 'U_statistic', 'p_value', 'p_bh', 'cohens_d', 'abs_cohens_d',
                 'direction', 'significant_raw', 'significant_bh']
    ...
    print(f"\n{'─'*80}")
    print(f"TOP 30 PAIRWISE RESULTS (sorted by p-value)")
    print(f"{'─'*80}")
    for i, row in results_df.head(30).iterrows():
        star = '***' if row['p_bh'] < 0.001 else '**' if row['p_bh'] < 0.01 else '*' if row['p_bh'] < 0.05 else '.' if row['p_value'] < 0.05 else ''
        ratio_tag = ' [RATIO]' if row['is_ratio'] else ''
        print(f"  p={row['p_value']:.4f} (q={row['p_bh']:.4f}) d={row['cohens_d']:+.2f} {star:4s} "
              f"{row['comparison']:30s} {row['feature']:25s} "
              f"{row['direction']:20s}{ratio_tag}")

    # ── Top multi-group results ─────────────────────────────────────────
    print(f"\n{'─'*80}")
    print(f"TOP 30 MULTI-GROUP / INTERACTION RESULTS")
    print(f"{'─'*80}")
    for i, row in mg_df.head(30).iterrows():
        star = '***' if row['p_bh'] < 0.001 else '**' if row['p_bh'] < 0.01 else '*' if row['p_bh'] < 0.05 else '.' if row['p_value'] < 0.05 else ''
        print(f"  p={row['p_value']:.4f} (q={row['p_bh']:.4f}) {star:4s} "
    ...
    if len(mg_sig) > 0:
        print(f"\n{'─'*80}")
        print(f"SIGNIFICANT MULTI-GROUP / INTERACTION (p<0.05)")
        print(f"{'─'*80}")
        for comp in sorted(mg_sig['comparison'].unique()):
            ...
            for _, row in comp_res.iterrows():
                ...
                if row['test_type'] == 'kruskal_wallis':
                    print(f"    p={row['p_value']:.4f} H={row['H_statistic']:.1f} eta²={row['eta_squared']:.3f} "
                          f"{row['feature']:25s} medians=[{row['group_medians']}]{bh_tag}")
                else:
                    print(f"    p={row['p_value']:.4f} {row['feature']:25s} ({row['groups']}){bh_tag}")

    # ── Significant pairwise by comparison group ────────────────────────
    print(f"\n{'─'*80}")
    print(f"SIGNIFICANT PAIRWISE (uncorrected p<0.05) BY COMPARISON GROUP")
    print(f"{'─'*80}")
    for comp in sorted(sig_raw['comparison'].unique()):
        ...
        for _, row in comp_results.iterrows():
            bh_tag = ' [BH]' if row['significant_bh'] else ''
            ratio_tag = ' [RATIO]' if row['is_ratio'] else ''
            print(f"    p={row['p_value']:.4f} d={row['cohens_d']:+.2f} {row['feature']:25s} {row['direction']}{bh_tag}{ratio_tag}")
    ...
    fig.suptitle('Comprehensive MK Analysis: All Tests × All Features', fontsize=16, fontweight='bold')

    # Panel 1: Pairwise volcano
    direct = results_df[~results_df['is_ratio']]
    ax = axes[0, 0]
    ax.scatter(direct['cohens_d'], -np.log10(direct['p_value']),
               alpha=0.15, s=10, c='gray')
    sig_mask = direct['p_value'] < 0.05
    ax.scatter(direct.loc[sig_mask, 'cohens_d'], -np.log10(direct.loc[sig_mask, 'p_value']),
               alpha=0.6, s=20, c='red', label=f'p<0.05 (n={sig_mask.sum()})')
    bh_mask = direct['p_bh'] < 0.05
    ax.scatter(direct.loc[bh_mask, 'cohens_d'], -np.log10(direct.loc[bh_mask, 'p_value']),
               alpha=0.9, s=50, c='darkred', marker='*', label=f'BH q<0.05 (n={bh_mask.sum()})')
    ax.axhline(-np.log10(0.05), color='red', linestyle='--', alpha=0.5)
    ax.set_xlabel("Cohen's d"); ax.set_ylabel('-log10(p)')
    ...
    if len(mg_df) > 0:
        ...
        if len(kw_rows) > 0:
            ax.scatter(np.random.uniform(-0.3, 0.3, len(kw_rows)),
                       -np.log10(kw_rows['p_value']),
                       alpha=0.3, s=15, c='steelblue', label=f'Kruskal-Wallis (n={len(kw_rows)})')
            kw_sig = kw_rows[kw_rows['p_value'] < 0.05]
            ax.scatter(np.random.uniform(-0.3, 0.3, len(kw_sig)),
                       -np.log10(kw_sig['p_value']),
                       alpha=0.8, s=30, c='blue', label=f'KW p<0.05 (n={len(kw_sig)})')

        if len(int_rows) > 0:
            ax.scatter(np.random.uniform(0.7, 1.3, len(int_rows)),
                       -np.log10(int_rows['p_value']),
                       alpha=0.3, s=15, c='orange', label=f'Interaction ART (n={len(int_rows)})')
            int_sig = int_rows[int_rows['p_value'] < 0.05]
            ax.scatter(np.random.uniform(0.7, 1.3, len(int_sig)),
                       -np.log10(int_sig['p_value']),
                       alpha=0.8, s=30, c='darkorange', label=f'Int p<0.05 (n={len(int_sig)})')
    ...
    if all_sig_features and all_sig_comps:
        ...
        im = ax.imshow(heat_data.values.astype(float), aspect='auto', cmap='YlOrRd',
    ...
    if len(mg_df) > 0:
        for tt in mg_df['test_type'].unique():
            ...
            summary_text.append(f"")
    summary_text.append(f"")
    summary_text.append(f"TOP SIGNIFICANT INTERACTIONS:")
    if len(mg_sig) > 0:
    ...
    if len(all_comp_counts) > 0:
        ...
        for comp in all_comp_counts.index:
            if 'Interaction' in comp: colors.append('#ff8800')
            elif 'KW_' in comp: colors.append('#4488cc')
            elif 'Ratio' in comp: colors.append('#8844aa')
            elif '_vs_' in comp: colors.append('#44aa44')
            elif '|' not in comp and 'KW' not in comp: colors.append('#cc4444')
            elif comp.count('_') >= 1 and '|' in comp: colors.append('#66bb66')
            else: colors.append('#cc8844')
        ...
        ax.set_title('Significant Features per Comparison (all test types)', fontsize=14, fontweight='bold')
        ax.invert_yaxis()
    ...
    if len(mg_sig) > 0:
        ...
        if len(int_sig) > 0:
            ...
            for _, row in int_sig.head(15).iterrows():
                bh_tag = ' [BH]' if row['significant_bh'] else ''
                print(f"    {row['comparison']:35s} {row['feature']:25s} p={row['p_value']:.4f}{bh_tag}")
        ...
        if len(kw_sig) > 0:
            ...
            for _, row in kw_sig.head(15).iterrows():
                bh_tag = ' [BH]' if row['significant_bh'] else ''
                print(f"    {row['comparison']:35s} {row['feature']:25s} p={row['p_value']:.4f} H={row['H_statistic']:.1f}{bh_tag}")
    ...
    themes = {
        ...
        '4-group pairwise': sig_raw[sig_raw['comparison'].str.contains('_vs_') & ~sig_raw['is_ratio']],
    }
    ...
    for theme, theme_df in themes.items():
        if len(theme_df) > 0:
            ...
            for _, row in theme_df.head(10).iterrows():
                bh_tag = ' [BH]' if row['significant_bh'] else ''
                print(f"    {row['comparison']:35s} {row['feature']:25s} p={row['p_value']:.4f} d={row['cohens_d']:+.2f} {row['direction']}{bh_tag}")
        else:
