...
def run_rf_embedding(
    ...
    *,
    ...
    logger.info(
        f"Feature matrix: {X.shape[0]:,} x {X.shape[1]} (from {len(detections):,} detections)"
    )
    ...
    logger.info(
        f"Running UMAP on leaf indices (metric=hamming, "
...
def run_morph_umap(
    ...
    if len(all_features) < 3:
        logger.warning(
            f"Only {len(all_features)} morph features found, need >= 3. Skipping morph UMAP."
        )
        return detections
...
def run_spatial_network(
    ...
    *,
    ...
    if marker_filter is not None and marker_filter.strip():
        ...
        logger.info(
            f"Marker filter '{marker_filter}': {len(detections):,} -> {len(filtered_idx):,}"
        )
    else:
    ...
    if len(filtered_idx) < 3:
        logger.warning(
            f"Only {len(filtered_idx)} cells after filtering, need >= 3. Skipping network."
        )
        return detections
    ...
    if len(positions) < 3:
        logger.warning(
            f"Only {len(positions)} cells with valid positions, need >= 3. Skipping network."
        )
        return detections
    ...
    logger.info(
        f"  Delaunay edges: {n_total_edges:,} total, {n_kept:,} kept "
    ...
    logger.info(
        f"  Connected components: {len(components):,} total, "
