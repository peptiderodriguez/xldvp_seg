...
def hopkins_statistic(X, n_samples=None, rng=None):
    ...
    Args:
        X: (n, d) feature matrix (should be scaled)
        n_samples: number of test points (default: min(200, n//2))
    ...
    # Random uniform points in the data bounding box
    mins = X.min(axis=0)
    maxs = X.max(axis=0)
...
def find_optimal_k(X, max_k=8, rng=None):
    """Find optimal k for k-means via elbow method (kneedle-like).

    Runs k=2..max_k, computes inertia (WCSS) for each, picks k at the
    maximum perpendicular distance from the line joining the curve's endpoints.
    ...
    # Subsample for kmeans fit speed if large; always predict on full X
    fit_X = X
    if n > 10000:
    ...
    for k in range(2, max_k + 1):
        ...
        if n > 10000:
            km.fit(fit_X)
            labels = km.predict(X)
            # Recompute inertia on full X for consistency across k values
    ...
    # Elbow: normalize (k, inertia) to [0,1], pick k with max perpendicular
    ...
    if len(ks) == 1:
        ...
        # Inertia is monotone decreasing with k — normalize so start=1, end=0
...
def cluster_leiden(X, *, n_neighbors=15, resolution=1.0, seed=42):
    ...
    # Skip self-neighbor (column 0) and build undirected edge list
...
def cluster_hdbscan(X, *, min_cluster_size=50):
    """HDBSCAN on the given space. Returns labels (-1 = noise)."""
    import hdbscan

    n = X.shape[0]
    mcs = max(5, min(min_cluster_size, n // 5))  # cap so we don't request too many
...
def process_region(
    ...
    """Process a single region: PCA (→ var_cutoff) + UMAP + clustering metrics.

    Clustering metrics computed on the PCA-reduced space (UMAP distorts distances).
    UMAP is used for visualization only.

    Returns dict with coords, cluster labels, metrics, or None if too few cells.
    """
    from umap import UMAP

    X, _, valid_idx = _extract_feature_matrix(detections, feature_names)
    if X is None or X.shape[0] < 50:
        return None

    # Drop constant features (zero variance → NaN after StandardScaler)
    ...
    # Full PCA to find how many components hit var_cutoff
    ...
    n_pcs = min(n_for_cutoff, max_pcs, full_max)
    n_pcs = max(n_pcs, 2)  # need at least 2 for UMAP

    # Transform once with all needed components (reuse pca_full)
    X_pca = pca_full.transform(X_scaled)[:, :n_pcs]

    # Subsample BEFORE UMAP for speed (UMAP on 5000 pts ~2-3s; on 50K ~30s+)
    ...
    umap = UMAP(
        ...
        random_state=42,
        n_jobs=1,  # random_state + n_jobs=1 for reproducibility
    )
    X_umap = umap.fit_transform(X_pca_sub)

    # Clustering metrics on PCA-reduced subsample (principled distance space)
    hopkins = hopkins_statistic(X_pca_sub, rng=rng)
    best_k, sil, labels_kmeans, ch, sil_per_k, inertia_per_k = find_optimal_k(
        X_pca_sub, max_k=max_k, rng=rng
    )

    # Leiden on PCA-kNN graph (scverse-standard)
    try:
    ...
    # HDBSCAN on PCA space (principled, density-based)
    try:
    ...
    # HDBSCAN on UMAP (matches visual lobes, less principled)
    try:
    ...
    # Variance explained by first 3 PCs (for display, padded to 3)
    ...
    return {
        ...
        # Primary: kmeans (keeps backward-compat with old viewer using `labels`)
