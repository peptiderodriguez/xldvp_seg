...
Functions for preparing NMJ contours for laser microdissection:
1. Dilation - add buffer around contour so laser cuts outside the NMJ
2. RDP simplification - reduce point count for LMD hardware limits
3. Validation - fix invalid polygons (self-intersections, etc.)

Usage:
    from contour_processing import process_contour, process_contours_batch
...
# Default parameters
DEFAULT_PIXEL_SIZE_UM = 0.1725  # µm per pixel
DEFAULT_DILATION_UM = 0.5      # Dilate by 0.5µm
...
def process_detection_contours(
    ...
    if verbose:
        ...
        print(f"  Dilation: +{dilation_um} µm")
    ...
    if verbose:
        ...
        print(f"  Area change: {results['mean_area_before_um2']:.1f} → {results['mean_area_after_um2']:.1f} µm² ({results['area_increase_pct']:+.1f}%)")
...
if __name__ == '__main__':
    ...
    print(f"Area: {stats['area_before_um2']:.2f} → {stats['area_after_um2']:.2f} µm²")
