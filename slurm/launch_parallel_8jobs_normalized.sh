#!/bin/bash
# Launch 8 parallel jobs with CROSS-SLIDE NORMALIZATION enabled
# Each job processes 2 slides
...
echo "Submitting 8 parallel jobs WITH CROSS-SLIDE NORMALIZATION..."
echo "Output directory: $OUTPUT_DIR"

for i in {0..7}; do
    SLIDES="${SLIDE_PAIRS[$i]}"
    JOB_NAME="mkseg_norm_batch$((i+1))"
...
echo ""
echo "All 8 jobs submitted WITH NORMALIZATION!"
echo "Monitor with: squeue -u \$USER"
echo ""
echo "NOTE: Each batch normalizes its 2 slides together. For true cross-slide"
echo "      normalization across all 16 slides, consider processing them in a single job."
