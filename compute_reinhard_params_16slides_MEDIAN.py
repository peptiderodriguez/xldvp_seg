#!/usr/bin/env python3
"""
Compute MEDIAN-based Reinhard normalization parameters from 16 slides using tissue-aware sampling.

Uses MEDIAN and MAD (Median Absolute Deviation) instead of MEAN and STD for robustness to outliers.
This prevents dark/bright artifacts from skewing the reference statistics.
...
from segmentation.preprocessing.stain_normalization import compute_reinhard_params_from_samples_MEDIAN
...
def sample_pixels_from_tissue_tiles(czi_path, channel=0, n_samples=500000, tile_size=3000, block_size=512):
    ...
    try:
        ...
        logger.info(f"  Collected {len(all_samples):,} pixels (median intensity: {np.median(all_samples):.1f})")
...
def main():
    logger.info("="*70)
    logger.info("COMPUTING MEDIAN-BASED REINHARD PARAMETERS (16 SLIDES)")
    logger.info("="*70)
    logger.info("Using MEDIAN and MAD for robustness to outliers")
    logger.info("")
    ...
    # Compute MEDIAN-based Reinhard parameters in Lab color space
    logger.info("="*70)
    logger.info("COMPUTING MEDIAN-BASED REINHARD PARAMETERS (Lab color space)...")
    params = compute_reinhard_params_from_samples_MEDIAN(all_samples)
    ...
    # Save parameters
    output_file = Path("/viper/ptmp2/edrod/xldvp_seg_fresh/reinhard_params_16slides_MEDIAN.json")
    with open(output_file, 'w') as f:
        json.dump(params, f, indent=2)

    logger.info("="*70)
    logger.info("MEDIAN-BASED REINHARD PARAMETERS (Lab color space):")
    logger.info(f"  L channel: median={params['L_median']:.2f}, mad={params['L_mad']:.2f}")
    logger.info(f"  a channel: median={params['a_median']:.2f}, mad={params['a_mad']:.2f}")
    logger.info(f"  b channel: median={params['b_median']:.2f}, mad={params['b_mad']:.2f}")
    ...
    logger.info("\nNext step: Validate with median-based normalization")
