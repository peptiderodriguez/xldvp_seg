...
**Core scripts:** `run_segmentation.py` (detection), `run_lmd_export.py` (LMD XML), `train_classifier.py` (RF), `serve_html.py` (viewer). **SLURM launcher:** `scripts/run_pipeline.sh` (YAML-driven). **CLI:** `xlseg` with 14 subcommands.
