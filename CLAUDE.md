...
- `pipeline/` — 12 modules orchestrated by `run_segmentation.py`: cli, preprocessing, detection_{setup,loop}, shm_setup, samples, resume, post_detection, multigpu_phase4, finalize, server, background. DAG: detection_loop→shm_setup, post_detection→background.
...
- `analysis/` — 10 modules promoted from scripts for programmatic use: `marker_classification`, `cluster_features` (ClusteringConfig dataclass), `spatial_network`, `pattern_detection`, `sliding_window_sampling`, `aggregation`, `nuclear_count`, `omic_linker` (supports dvp-io), `vessel_characterization`, `region_segmentation`.
