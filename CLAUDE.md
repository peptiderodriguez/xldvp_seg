...
**Preprocessing caches** (colocated in `<slide_output_dir>/` by default, or in `--flat-field-cache-dir <path>` for cross-run sharing on the same CZI):

- **`flat_field_profile.npz`** — the slide-wide illumination estimate (~1-2h on 5-channel whole-mouse). Applies to fluorescence only.
- **`tissue_filter.json`** — `(variance_threshold, tissue_tiles)` from calibration + filter scan (~3-4 min). Benefits fluorescence **and** brightfield.

Cache key includes CZI `(path, mtime, size)` + `scene` + algorithm-specific fields (flat-field: channels + slide shape + photobleach flag; tissue: tile_size + tile_overlap + tissue_channel + modality + manual_threshold + n_all_tiles) + `ALGORITHM_VERSION`. Any change invalidates. Pre-scene caches (written before scene joined the key) are treated as scene=0 for back-compat so in-flight jobs never redundantly recompute after an upgrade. POSIX `O_CREAT|O_EXCL` advisory lock (`flat_field_profile.computing` / `tissue_filter.computing`) ensures exactly one shard recomputes on cold start; others wait and load. 3h stale-lock recovery guards against crashed computes. Disable flat-field caching by passing `--no-normalize-features` (which skips flat-field entirely). Shared primitives live in `xldvp_seg.utils.cache_utils`.
