...
| `--max-area-change-pct` | 10.0 | Adaptive RDP: max symmetric-difference deviation (%). 0 = use fixed `--rdp-epsilon`. |
| `--max-dilation-area-pct` | 10.0 | Adaptive dilation: max area increase (%) for laser buffer. 0 = use fixed `--dilation-um`. |
