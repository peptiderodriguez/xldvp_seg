...
- **Tile overlap:** default **0.15** (77 µm zones, covers polyploid hepatocytes up to ~155 µm). Bump to `tile_overlap: 0.25` in YAML (or `--tile-overlap 0.25`) for MK-scale cells (≥100 µm, multinucleated giants). Smaller overlap bisects large cells at tile edges — dedup only merges TRUE duplicates, not bisected cells, so you end up with clipped contours. Cross-tile merging exists only for vessel strategy today. Observed: 25% overlap on 16 MK slides recovered ~10% more detections vs 10%.
