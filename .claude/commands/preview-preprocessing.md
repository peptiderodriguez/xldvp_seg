...
- CZI file path
- Channel: can be an index (e.g., 1), wavelength (e.g., 647), marker name (e.g., SMA), or "all". If the user gives a name/wavelength, resolve it using `parse_markers_from_filename()` + `resolve_channel_indices()` from `xldvp_seg.io.czi_loader`.
