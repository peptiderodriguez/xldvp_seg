...
def build_parser():
    ...
    # Output
    parser.add_argument(
        "--output-dir", type=str, default=None, help="Output directory (default: ./output)"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Fraction of tissue tiles to process (default: 100%%)",
    )
    parser.add_argument(
        "--channel",
        type=int,
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Directory to read/write per-slide preprocessing caches: "
            "flat_field_profile.npz (illumination profile, ~1-2h recompute) + "
            "tissue_filter.json (variance threshold + tissue-tile list, ~3-4 min "
            "recompute). Default: both caches sit inside each run's "
            "slide_output_dir so they are scoped to that run + its --resume "
            "siblings. Pass a slide-level path (e.g. /path/to/caches/<slide_stem>/) "
            "to share caches across detection runs with different --output-dir so "
            "parameter sweeps on the same CZI skip the recomputes."
        ),
    )
    parser.add_argument(
        "--html-normalization",
    ...
    parser.add_argument(
        "--mk-min-area", type=float, default=200.0, help="Minimum MK area in um^2 (default 200)"
    )
    parser.add_argument(
        "--mk-max-area", type=float, default=2000.0, help="Maximum MK area in um^2 (default 2000)"
    )
    parser.add_argument(
        "--refine-masks",
        action="store_true",
    ...
    # Vessel parameters
    parser.add_argument(
        "--min-vessel-diameter", type=float, default=10, help="Minimum vessel outer diameter in um"
    )
    parser.add_argument(
        "--max-vessel-diameter",
        type=float,
    ...
    parser.add_argument(
        "--min-wall-thickness", type=float, default=2, help="Minimum vessel wall thickness in um"
    )
    parser.add_argument(
        "--max-aspect-ratio",
        type=float,
    ...
    parser.add_argument(
        "--min-ribbon-width", type=float, default=5, help="Minimum expected ribbon width in um"
    )
    parser.add_argument(
        "--max-ribbon-width", type=float, default=30, help="Maximum expected ribbon width in um"
    )
    parser.add_argument(
        "--min-fragment-area",
        type=float,
    ...
    parser.add_argument(
        "--no-serve", action="store_true", help="Do not start server after processing"
    )
    ...
    parser.add_argument(
        "--stop-server", action="store_true", help="Stop any running background server and exit"
    )
    parser.add_argument(
        "--server-status",
        action="store_true",
...
def postprocess_args(args, parser):
    ...
    if args.channel is None:
        if args.cell_type == "nmj":
            ...
            try:
                ...
                parser.error(
                    f"--tp-detection-channels: first entry must be integer, got '{args.tp_detection_channels}'"
                )
            ...
            try:
                ...
                parser.error(
                    f"--cellpose-input-channels: first entry must be integer, got '{args.cellpose_input_channels}'"
                )
    ...
    if args.cell_type == "islet":
        ...
        for pair in args.islet_marker_channels.split(","):
            ...
            if ":" not in pair:
                parser.error(
                    f"--islet-marker-channels: each entry must be NAME:CHANNEL, got '{pair}'"
                )
            ...
            try:
                ...
                parser.error(
                    f"--islet-marker-channels: channel must be integer, got '{ch.strip()}' in '{pair}'"
                )
    ...
    if args.tile_shard is not None:
        try:
            ...
            if shard_idx < 0 or shard_idx >= shard_total or shard_total < 1:
                parser.error(
                    f"--tile-shard: INDEX must be 0..TOTAL-1, got {shard_idx}/{shard_total}"
                )
        ...
        if not args.resume and not args.resume_from:
            print(
                "WARNING: --tile-shard without --resume: each node will create its own directory. "
                "Use --resume <shared-dir> so all shards write to the same location.",
                flush=True,
            )
    ...
    if args.resume:
        ...
        if shard_manifests and not args.merge_shards:
            print(
                f"Auto-detected {len(shard_manifests)} shard manifests -- enabling --merge-shards",
                flush=True,
            )
