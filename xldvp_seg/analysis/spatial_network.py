"""Spatial cell analysis: RF embedding, morphological UMAP, and Delaunay network.

Reusable functions extracted from ``scripts/spatial_cell_analysis.py``.

Public API
----------
- :func:`parse_marker_filter` — parse "feature_name==value" filter strings
- :func:`run_rf_embedding` — RF leaf-node co-occurrence UMAP
- :func:`run_morph_umap` — morphological feature UMAP (small multiples)
- :func:`run_spatial_network` — Delaunay adjacency graph + Louvain communities

All heavy dependencies (sklearn, scipy, networkx, umap-learn, matplotlib)
...
def _get_positions_um(detections, pixel_size):
    """Extract cell positions in microns.

    Delegates to :func:`extract_positions_um` with ``return_indices=True``
    for consistent position resolution and valid-index tracking.

    Returns ``(positions, valid_indices)`` where *positions* is ``(n, 2)``
    in ``[x, y]`` order.
    """
    all_positions, _, valid_indices = extract_positions_um(
        detections, pixel_size_um=pixel_size, return_indices=True
    )
    return all_positions, valid_indices


def _subsample_for_umap(max_samples, n_samples, rng=None):
...
def _run_umap(X, fit_idx, all_idx, n_neighbors=30, min_dist=0.1, metric="euclidean"):
    ...
    if len(fit_idx) == len(all_idx):
        ...
        logger.info(
            "  Fitting UMAP on %s / %s samples...", f"{len(fit_idx):,}", f"{len(all_idx):,}"
        )
...
def parse_marker_filter(filter_str):
    ...
    Raises:
        ValueError: If *filter_str* cannot be parsed.
    """
