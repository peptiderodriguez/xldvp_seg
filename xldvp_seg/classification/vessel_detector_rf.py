...
class VesselDetectorRF(BaseVesselClassifier):
    """
    ...
    def __init__(
        ...
        super().__init__(
            ...
            default_features=VESSEL_DETECTION_FEATURES,
            use_scaler=False,  # RF is scale-invariant, no scaler needed
        )
    ...
    def _extract_diameter_from_dict(self, features_dict: dict[str, Any]) -> float:
        """
    ...
    def _prepare_training_data(
        ...
        y: np.ndarray | list[str] | list[bool],
        feature_names: list[str] | None = None,
    ...
    def train(
        ...
        if stratify_by_size and size_classes is not None:
            ...
            cv_scores = cross_val_score(
                ...
                X_array,
                y_encoded,
                cv=cv.split(X_array, stratify_groups),
                scoring="accuracy",
            )
        else:
            cv = StratifiedKFold(n_splits=cv_folds, shuffle=True, random_state=self.random_state)
    ...
    def train_from_files(
        ...
        if verbose:
            ...
            if unmatched_uids:
                logger.warning(
                    f"Unmatched UIDs: {unmatched_uids[:10]}{'...' if len(unmatched_uids) > 10 else ''}"
                )
    ...
    def predict_vessel(self, features: dict[str, Any] | np.ndarray) -> tuple[bool, float]:
        """
    ...
    def evaluate(
        ...
        report = classification_report(
            y_true, y_pred, target_names=["non_vessel", "vessel"], output_dict=True
        )
        ...
        if verbose:
            ...
            logger.info(
                "\n%s",
                classification_report(y_true, y_pred, target_names=["non_vessel", "vessel"]),
            )
            logger.info("\nConfusion Matrix:")
    ...
    @classmethod
    def load(cls, path: str | Path) -> "VesselDetectorRF":
        """
        ...
        if f1 is not None:
            ...
            logger.info("  F1 Score: N/A")
        return instance


def train_vessel_detector(
    annotations_json: str | Path, detections_json: str | Path, **kwargs
) -> VesselDetectorRF:
    """
...
def predict_vessel(model: VesselDetectorRF, features: dict[str, Any]) -> tuple[bool, float]:
    """
