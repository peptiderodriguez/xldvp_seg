    ...
    for ch in channels:
        ...
        for i in range(n_strips):
            ...
            strip = reader.read_mosaic(
                region=(x_start, y_start + y_off, width, h), scale_factor=1, C=ch
            )
...
def compute_cell_medians(detections, tiles_dir, ch_data, marker_map, x_start, y_start):
        ...
        for i in det_indices:
            ...
            for name, ch_idx in marker_map.items():
                if ch_idx not in ch_data:
                    continue
                ch_region = ch_data[ch_idx][
                    rel_ty + y_sl.start : rel_ty + y_sl.stop,
    ...
    logger.info(
        "  Computed medians for %d/%d cells across %d tiles",
        n_processed,
        len(detections),
        len(tile_groups),
    )
...
def find_islet_regions(
    ...
    logger.info(
        "  Tissue-level islet finding: %s downsampled image (%.2f um/px)",
        signal.shape,
        ds_pixel_size,
    )
    logger.info(
        "    Otsu: raw=%.3f x%s = %.3f, %d candidate regions",
        otsu_raw,
        otsu_multiplier,
        otsu_t,
        n_regions,
    ...
    logger.info(
        "    blur=%.1fpx close=%dpx min_area=%dpx buffer=%dpx",
        sigma_px,
        close_px,
        min_area_px,
        buffer_px,
...
def assign_cells_to_regions(detections, region_labels, downsample, x_start, y_start, min_cells=5):
    ...
    logger.info(
        "  Assigned %d/%d cells to %d islet regions " "(dropped %d regions with < %d cells)",
        n_assigned,
        len(detections),
        len(groups),
        len(small),
...
def save_region_diagnostic(
    endocrine_signal, region_labels, islet_features, quality_dropped_ids, ds_pixel_size, output_path
):
    ...
    ax.set_title(
        f"Tissue-level islet candidates: "
    ...
    ax.text(
        w - bar_px / 2 - 20, h - 40, "500 um", color="white", ha="center", va="bottom", fontsize=8
    )
...
def compute_spatial_metrics(cells, pixel_size, marker_map):
    ...
    try:
        ...
        result["cell_density_per_1000um2"] = (
            round(len(all_coords) / hull_area * 1000, 2) if hull_area > 0 else 0.0
        )
    except Exception:
...
def compute_homogeneity(cells, pixel_size, marker_map):
    ...
    for qx in [False, True]:
        for qy in [False, True]:
            q_mask = ((coords[:, 0] >= centroid[0]) == qx) & ((coords[:, 1] >= centroid[1]) == qy)
            n_q = int(q_mask.sum())
            if n_q < 3:
                continue
            for name in marker_map.keys():
    ...
    if variances:
        ...
        result["homogeneity_score"] = round(1.0 / (1.0 + 20.0 * float(np.mean(variances))), 3)
    return result
...
def analyze_all_islets(detections, islet_groups, pixel_size, marker_map, thresholds=None):
...
def compute_tissue_level(islet_features, detections, pixel_size):
    ...
    if len(indexed_centroids) >= 2:
        ...
        for j, orig_idx in enumerate(orig_indices):
            islet_features[orig_idx]["nn_islet_distance_um"] = round(float(nn_dists[j]), 1)
    ...
    if len(all_coords) >= 3:
        ...
        try:
            ...
            if tissue_area > 0:
                result["endocrine_area_fraction"] = round(total_islet_area / tissue_area, 4)
                tissue_area_mm2 = tissue_area / 1e6
                result["islet_density_per_mm2"] = round(len(islet_features) / tissue_area_mm2, 2)
            ...
            for f in islet_features:
                if "centroid_x_um" in f and "centroid_y_um" in f:
                    ...
                    f["dist_to_tissue_edge_um"] = round(-max_sd, 1) if max_sd <= 0 else 0.0
        except Exception:
            pass
    ...
    if len(islet_features) >= 5:
        ...
        frac_keys = [
            k
            for k in islet_features[0].keys()
...
def render_islet_card(
    ...
    for i, name in enumerate(marker_names):
        ...
        if cnt > 0:
            ...
            bar_parts.append(
                f'<div style="width:{100*cnt/total:.0f}%;background:{html_c}" '
                f'title="{name} {cnt} ({100*cnt/total:.0f}%)"></div>'
            )
    n_multi = islet_feat.get("n_multi", 0)
    if n_multi > 0:
        bar_parts.append(
            f'<div style="width:{100*n_multi/total:.0f}%;background:#ffaa00" '
            f'title="multi {n_multi}"></div>'
        )
    n_none = islet_feat.get("n_none", 0)
    if n_none > 0:
        bar_parts.append(
            f'<div style="width:{100*n_none/total:.0f}%;background:#555" '
            f'title="none {n_none}"></div>'
        )
    bar_html = (
        f'<div style="display:flex;height:10px;width:100%;border-radius:4px;'
    ...
    if n_border + n_core > 0:
        ...
        border_bar = (
            f'<div style="display:flex;height:6px;width:100%;border-radius:3px;'
    ...
    if flags:
        flag_html = " ".join(
            f'<span style="color:#ff6666;background:#330000;padding:1px 4px;'
            f'border-radius:3px;font-size:10px">{f}</span>'
            for f in flags
        )
...
def generate_html(
    ...
    for iid in islet_groups:
        ...
        coords = np.array([_get_global_center(c) or [0, 0] for c in cells])
    ...
    for feat in sorted_feats:
        ...
        cells = [c for c in all_cells if tuple(c.get("tile_origin", [-1, -1])) == (tx, ty)]
        if not cells:
            continue
    ...
    if tissue_stats:
        ...
        if t_parts:
            tissue_html = (
                f'<div class="summary" style="border-left:3px solid #ff00ff">'
                f'{"".join(t_parts)}</div>'
            )
...
def main():
    ...
    parser.add_argument(
        "--run-dir", required=True, help="Path to completed islet run output directory"
    )
    parser.add_argument(
        "--czi-path", required=True, help="Path to CZI file (for loading marker + display channels)"
    )
    parser.add_argument(
        "--buffer-um",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.5,
        help="Multiply Otsu threshold by this factor (>1 = stricter, " "default: 1.5)",
    )
    parser.add_argument(
        "--min-cells", type=int, default=5, help="Minimum cells per islet region (default: 5)"
    )
    parser.add_argument(
        "--display-channels",
        type=str,
    ...
    parser.add_argument(
        "--no-html", action="store_true", help="Skip HTML generation (just CSV + JSON)"
    )
    ...
    if args.pixel_size is not None:
        ...
        if czi_pixel_size is not None:
            logger.info(
                "  Overriding CZI pixel_size=%.4f with --pixel-size=%.4f",
                czi_pixel_size,
                pixel_size,
            )
        ...
        logger.error(
            "pixel_size must be provided via --pixel-size or read from "
    ...
    islet_groups = assign_cells_to_regions(
        detections, region_labels, ds, x_start, y_start, min_cells=args.min_cells
    )
    ...
    thresholds = classify_by_percentile(detections, marker_map, percentile=args.marker_percentile)
    ...
    islet_features = analyze_all_islets(
        detections, islet_groups, pixel_size, marker_map, thresholds=thresholds
    )
        ...
        if drop:
            ...
            for d in detections:
                if d.get("islet_id", -1) in quality_dropped_ids:
                    d["islet_id"] = -1
            islet_features = keep
            drop_info = ", ".join(f"{f['islet_id']}({f['_median_signal']:.3f})" for f in drop)
            logger.info(
                "  Quality filter (median tissue signal, cutoff=%.3f): "
            ...
            logger.info("  Quality filter: all %d islets above median=%.3f", n_before, cutoff)
    ...
    if islet_features:
        ...
        logger.info("  Cells: %d total, median %.0f/islet", sum(cells_per), np.median(cells_per))
        logger.info(
            "  Area: median %.0f um2, range %.0f-%.0f", np.median(areas), min(areas), max(areas)
        )
        ...
        mcis = [
            f.get("mantle_core_index")
            for f in islet_features
            if f.get("mantle_core_index") is not None
        ]
        if mcis:
