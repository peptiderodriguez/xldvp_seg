...
def dilate_label_array(label_array, dilation_px):
    ...
    if expand_mask.any():
        ...
        result[expand_mask] = label_array[nearest_idx[0][expand_mask], nearest_idx[1][expand_mask]]
...
def main():
    ...
    parser.add_argument(
        "--nuclei-dir", required=True, help="Run directory for nuclei-only segmentation"
    )
    parser.add_argument("--pm-dir", required=True, help="Run directory for PM+nuclei segmentation")
    parser.add_argument(
        "--dilation-px",
        type=int,
        default=None,
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Only compute stats, do not modify mask files"
    )
    ...
    print(
        f"Nuclei-only:  {stats['nuclei_count']:,} cells, "
    ...
    print(
        f"PM+nuclei:    {stats['pm_count']:,} cells, "
    ...
    print(
        f"Radius: nuclei={stats['nuclei_median_radius_px']:.1f} px, "
    ...
    for i, td in enumerate(tile_dirs):
        ...
        if (i + 1) % 5 == 0 or i == len(tile_dirs) - 1:
            print(
                f"  Processed {i+1}/{len(tile_dirs)} tiles "
