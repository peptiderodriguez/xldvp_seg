...
def load_and_prepare_data(score_threshold=0.75):
    ...
    if n_filtered:
        print(
            f"  Score filter: {n_total} total, {n_filtered} below {score_threshold}, {len(rows)} kept"
        )
...
def _draw_interaction(
    ax, df, feat, factor_x, factor_line, factor_facet=None, p_value=None, title_label=None
):
...
def generate_interaction_plots(df, features, anova_results, output_path):
    ...
    fig, axes = plt.subplots(n_feats, n_cols, figsize=(16, 3.5 * n_feats), squeeze=False)
    fig.suptitle(
        "ART Interaction Analysis: MK Morphological Features\n"
    ...
    for ri, feat in enumerate(plot_features):
        ...
        for ci, (effect_key, fx, fl, ff, label) in enumerate(interaction_configs):
            ...
            _draw_interaction(
                ax, df, feat, fx, fl, ff, p_value=p_val, title_label=f"{feat}\n{label}"
            )
            if ci == 0:
...
def generate_anova_summary_figure(results_df, features, output_path):
    ...
    fig, (ax1, ax2) = plt.subplots(
        1, 2, figsize=(16, max(5, len(feat_order) * 0.6)), gridspec_kw={"width_ratios": [3, 2]}
    )
    ...
    im1 = ax1.imshow(
        ...
        vmin=0,
        vmax=max(3, np.nanmax(log_p) * 1.1),
    )
    ...
    for i in range(len(feat_order)):
        for j in range(len(ALL_EFFECTS)):
            ...
            txt = (
                "***"
                if p < 0.001
                else "**" if p < 0.01 else "*" if p < 0.05 else "\u2020" if p < 0.10 else ""
            )
            if txt:
                ax1.text(
                    j,
    ...
    im2 = ax2.imshow(
        ...
        interpolation="nearest",
        vmin=0,
        vmax=max(0.5, np.nanmax(eta_vals) * 1.1),
    )
    ...
    plt.suptitle(
        "MK -- ART ANOVA Decomposition (Sex x Treatment x Bone)", fontsize=14, fontweight="bold"
    )
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches="tight")
    plt.close()
    ...
    for feat in features:
        ...
        for sex in ["F", "M"]:
            ...
            for bone in ["femur", "humerus", "collapsed"]:
                if bone == "collapsed":
                    slide_means = (
                        agg_df[agg_df["sex"] == sex]
                        .groupby(["slide", "treatment"])[feat]
                        .mean()
                        .reset_index()
                    )
                ...
                delta = (
                    hu_mean - gc_mean if not (np.isnan(gc_mean) or np.isnan(hu_mean)) else np.nan
                )
                pct = (
                    (delta / abs(gc_mean) * 100)
                    if (not np.isnan(delta) and abs(gc_mean) > 1e-9)
                    else np.nan
                )
...
def generate_summary_dashboard(agg_df, features, anova_results, results_df, output_path):
    ...
    gs = fig.add_gridspec(
        3, 3, hspace=0.42, wspace=0.35, left=0.07, right=0.95, top=0.92, bottom=0.06
    )
    fig.suptitle(
        "MK Response to Hindlimb Unloading: Two-Pathway Regulation", fontsize=14, fontweight="bold"
    )
    panels = "ABCDEFGHI"
    ...
    for ci, feat in enumerate(KEY_FEATS):
        ...
        for bone in ["femur", "humerus"]:
            ...
            for sex in ["F", "M"]:
                means, ses = [], []
                for trt in ["GC", "HU"]:
                    vals = bone_data[(bone_data["sex"] == sex) & (bone_data["treatment"] == trt)][
                        feat
                    ].dropna()
                ...
                ax.errorbar(
                    ...
                    linestyle=bs["linestyle"],
                    capsize=4,
                    label=f'{sex} {bs["label_suffix"]}',
                    zorder=3,
                )
            ...
            for sex in ["F", "M"]:
                for ti, trt in enumerate(["GC", "HU"]):
                    vals = (
                        bone_data[(bone_data["sex"] == sex) & (bone_data["treatment"] == trt)][feat]
                        .dropna()
                        .values
                    )
                    if len(vals) > 0:
        ...
        for sex in ["F", "M"]:
            for bone in ["femur", "humerus"]:
                ...
                ax.annotate(
                    ...
                    arrowprops=dict(
                        arrowstyle="->", color=LINE_COLORS[sex], lw=0.8, shrinkA=2, shrinkB=2
                    ),
                    ha="center",
                    va="center",
                )
        ...
        for eff_name in ["treatment", "sex", "sex:treatment", "treatment:bone"]:
            ...
            if len(r) > 0:
                ...
                if p < 0.10:
                    ...
                    eff_short = (
                        eff_name.replace("treatment", "Trt")
                        .replace("sex", "Sex")
                        .replace("bone", "Bone")
                        .replace(":", "\u00d7")
                    )
        ...
        if sig_texts:
            ax.text(
                ...
                ha="left",
                bbox=dict(
                    boxstyle="round,pad=0.3", facecolor="lightyellow", edgecolor="#ccc", alpha=0.9
                ),
            )

        ax.set_xticks([0, 1])
        ax.set_xticklabels(["GC", "HU"], fontsize=10)
        ax.set_title(f"{panels[ci]}: {FEAT_LABELS[feat]}", fontsize=11, fontweight="bold")
        ax.legend(fontsize=7, loc="best", ncol=2, handlelength=2.5, columnspacing=1)
    ...
    ax_d.set_title(
        f"{panels[3]}: Pathway Decomposition \u2014 Systemic vs Local",
        fontsize=11,
        fontweight="bold",
    )
    ...
    ax_e.set_title(f"{panels[4]}: Effect Hierarchy (area)", fontsize=11, fontweight="bold")
    # Add p-value annotations
    for i, (eta, p) in enumerate(zip(etas, ps)):
        stars = (
            "***"
            if p < 0.001
            else "**" if p < 0.01 else "*" if p < 0.05 else "\u2020" if p < 0.10 else "n.s."
        )
        ax_e.text(
            eta + 0.01,
            i,
    ...
    ax_f.set_title(f"{panels[5]}: Bone concordance (morphology)", fontsize=11, fontweight="bold")
    ...
    ax_g.set_title(
        f"{panels[6]}: Size vs Density \u2014 Pathway Dissociation", fontsize=11, fontweight="bold"
    )
    ax_g.legend(
        handles=[
    ...
    im = ax_h.imshow(
        ...
        interpolation="nearest",
        vmin=0,
        vmax=max(0.5, np.nanmax(eta_mat) * 1.1),
    )
    ...
    ax_h.set_title(f"{panels[7]}: Partial \u03b7\u00b2 (ART ANOVA)", fontsize=11, fontweight="bold")
...
def main():
    ...
    parser.add_argument(
        "--detections-bone", type=Path, default=None, help="Override bone-assigned detections JSON"
    )
    parser.add_argument(
        "--no-exclude", action="store_true", help="Include all slides (no exclusions)"
    )
    parser.add_argument(
        "--tissue-areas", type=Path, default=None, help="Override tissue areas JSON"
    )
    ...
    for s in sorted(agg_df["sex"].unique()):
        for t in sorted(agg_df["treatment"].unique()):
            for b in sorted(agg_df["bone"].unique()):
                n = (
                    (agg_df["sex"] == s) & (agg_df["treatment"] == t) & (agg_df["bone"] == b)
                ).sum()
    ...
    for _, r in mains.head(20).iterrows():
        ...
        print(
            f"  F({r['art_df_effect']:.0f},{r['art_df_resid']:.0f})={r['art_F']:6.2f}  "
