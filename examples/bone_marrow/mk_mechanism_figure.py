...
def draw_cell(
    ax, cx, cy, a_um, b_um, um_scale, fill, edge, lw=2.5, alpha=0.8, linestyle="-", zorder=3
):
...
def main():
    ...
    ax.text(
        0.5,
        0.99,
        "MK Morphology & Density Under Hindlimb Unloading",
        ha="center",
        va="top",
        fontsize=16,
        fontweight="bold",
    ...
    for ri, sex in enumerate(sexes):
        for ci, bone in enumerate(bones):
            ...
            ax.text(
                cell_cx,
                cy + max(hu_h, gc_b * 2 * UM_SCALE) / 2 + 0.015,
            ...
            ax.text(
                bar_x - bar_w / 2 - 0.004,
                cy - bar_max_h / 2 - 0.018,
                f'{gc_real["density"]:.1f}',
                ha="center",
                va="top",
                fontsize=7.5,
                color=COL_GC,
    ...
    ax.text(0.115, 0.065, "GC baseline (true scale)", va="center", fontsize=9, color=COL_GC)
    ax.add_patch(
        Ellipse(
            (0.09, 0.035),
            0.018,
            0.014,
            facecolor=COL_F_FILL,
            edgecolor=COL_F,
            linewidth=2,
            alpha=0.7,
        )
    ...
    ax.text(
        0.115, 0.035, f"HU (differences ×{EXAGGERATION:.0f})", va="center", fontsize=9, color="#333"
    )
    ...
    ax.text(
        0.44,
        0.065,
        "Systemic (parallel in both bones)",
        fontsize=9,
        color=COL_SYSTEMIC,
        va="center",
    )
    ...
    ax.text(
        0.44, 0.035, "Local mechanical (bone-specific)", fontsize=9, color=COL_LOCAL, va="center"
    )
    ...
    ax.plot(
        [sb_x + sb_len, sb_x + sb_len], [sb_y - 0.006, sb_y + 0.006], color="black", linewidth=2
    )
    ax.text(
        sb_x + sb_len / 2,
        sb_y - 0.015,
        f"{scale_um} µm (GC scale)",
        ha="center",
        va="top",
        fontsize=8,
        fontweight="bold",
    ...
    ax_m.text(
        0.5,
        0.97,
        "Two Dissociable Mechanosensing Pathways",
        ha="center",
        va="top",
        fontsize=14,
        fontweight="bold",
    ...
    ax_m.plot([0.50, 0.50], [0.00, 0.90], color="#CCC", linewidth=1.5, linestyle=":", zorder=0)
    ...
    ax_m.text(
        ...
        fontweight="bold",
        color=COL_LOCAL,
        bbox=dict(boxstyle="round,pad=0.3", facecolor="#FFF8E8", edgecolor=COL_LOCAL, linewidth=2),
    )
    ax_m.text(
        lx,
        0.72,
    ...
    ax_m.text(
        ...
        linespacing=1.3,
        bbox=dict(
            boxstyle="round,pad=0.5", facecolor="#FFFAE6", edgecolor=COL_LOCAL, linewidth=1.5
        ),
    )
    ...
    for bx, blabel, acol, alabel, ay1, ay2 in [
        ...
        ax_m.annotate(
            ...
            xy=(bx, ay1),
            xytext=(bx, ay2),
            arrowprops=dict(arrowstyle="->", color=acol, linewidth=2.5, mutation_scale=15),
        )
        ty = ay2 - 0.025 if ay2 < ay1 else ay2 + 0.015
        ax_m.text(bx, ty, alabel, ha="center", fontsize=8, fontweight="bold", color=acol)
    ...
    ax_m.text(
        ...
        color=COL_SYSTEMIC,
        bbox=dict(
            boxstyle="round,pad=0.3", facecolor="#F0EDFF", edgecolor=COL_SYSTEMIC, linewidth=2
        ),
    )
    ax_m.text(
        rx,
        0.72,
        "Controls: MK MORPHOLOGY",
        ha="center",
        fontsize=11,
        fontweight="bold",
        color=COL_SYSTEMIC,
    ...
    ax_m.text(
        ...
        linespacing=1.3,
        bbox=dict(
            boxstyle="round,pad=0.5", facecolor="#F5F2FF", edgecolor=COL_SYSTEMIC, linewidth=1.5
        ),
    )
    ...
    ax_m.text(
        ...
        color=COL_SYSTEMIC,
        bbox=dict(
            boxstyle="circle,pad=0.12", facecolor="#E8E0FF", edgecolor=COL_SYSTEMIC, linewidth=2
        ),
    )
    for sign in [-1, 1]:
        ax_m.annotate(
            "",
            xy=(rx + sign * 0.04, 0.09),
            xytext=(rx + sign * 0.02, 0.09),
            arrowprops=dict(arrowstyle="->", color=COL_SYSTEMIC, linewidth=2, mutation_scale=12),
        )
    ax_m.text(
        rx,
        -0.01,
