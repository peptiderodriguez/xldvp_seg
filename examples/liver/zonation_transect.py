...
def plot_zonation_map(positions, scores, cv_centroids, pv_centroids,
                      output_dir, title='Auto-detected zonation', paths=None,
                      corridor_um=150):
    """Spatial scatter plot colored by zonation score with CV-PV paths."""
    fig, ax = plt.subplots(figsize=(14, 10))

    sc = ax.scatter(positions[:, 0], positions[:, 1], c=scores,
                    cmap='RdYlBu_r', s=0.5, alpha=0.6, vmin=0, vmax=1,
                    rasterized=True)
    ...
    if paths:
        for pi, p in enumerate(paths):
            ...
            ax.plot(wp[:, 0], wp[:, 1], color='white', linewidth=2.0,
                    alpha=0.9, solid_capstyle='round', zorder=6)
            ax.plot(wp[:, 0], wp[:, 1], color='#00ff00', linewidth=1.0,
                    alpha=0.8, zorder=7)

    # Mark landmarks
    if len(cv_centroids) > 0:
        ax.scatter(cv_centroids[:, 0], cv_centroids[:, 1],
                   c='blue', s=100, marker='*', edgecolors='white',
                   linewidths=0.5, zorder=10, label=f'CV ({len(cv_centroids)})')
    if len(pv_centroids) > 0:
        ax.scatter(pv_centroids[:, 0], pv_centroids[:, 1],
                   c='red', s=100, marker='*', edgecolors='white',
                   linewidths=0.5, zorder=10, label=f'PV ({len(pv_centroids)})')
...
def plot_zone_map(positions, scores, cv_centroids, pv_centroids,
                  output_dir, paths=None, n_zones=3):
    ...
    ax.scatter(positions[:, 0], positions[:, 1], c=zone_labels,
               cmap=cmap, s=1.0, alpha=0.6, vmin=-0.5, vmax=n_zones - 0.5,
    ...
    if paths:
        for p in paths:
            wp = p['waypoints']
            ax.plot(wp[:, 0], wp[:, 1], color='white', linewidth=0.6,
                    alpha=0.25, solid_capstyle='round', zorder=4)

    # --- Vein landmarks ---
    if len(cv_centroids) > 0:
        ax.scatter(cv_centroids[:, 0], cv_centroids[:, 1],
                   c='#60b0ff', s=80, marker='o', edgecolors='white',
                   linewidths=1.0, zorder=10)
        for i, (x, y) in enumerate(cv_centroids):
            ax.annotate('CV', xy=(x, y), fontsize=4.5, fontweight='bold',
                        color='white', ha='center', va='center', zorder=11)
    if len(pv_centroids) > 0:
        ax.scatter(pv_centroids[:, 0], pv_centroids[:, 1],
                   c='#ff6060', s=80, marker='s', edgecolors='white',
                   linewidths=1.0, zorder=10)
        for i, (x, y) in enumerate(pv_centroids):
            ax.annotate('PV', xy=(x, y), fontsize=4.5, fontweight='bold',
                        color='white', ha='center', va='center', zorder=11)
    ...
    # --- Legend ---
    patches = [mpatches.Patch(facecolor=zone_colors[i], label=zone_names[i],
                              edgecolor='gray') for i in range(min(n_zones, len(zone_names)))]
    ...
    if paths:
        patches.append(plt.Line2D([0], [0], color='white', linewidth=1.5, alpha=0.5,
                                  label=f'Routes ({len(paths)})'))
    ...
    ax.set_title(f'Hepatic Zonation — {len(cv_centroids)} CV, {len(pv_centroids)} PV, '
...
def generate_auto_paths(cv_centroids, pv_centroids, max_path_length=2000,
                        all_pairs=False):
    ...
    logger.info(f"Generated {len(paths)} auto paths ({'all pairs' if all_pairs else 'nearest 1:1'}, "
...
def main():
    ...
    parser.add_argument('--corridor-um', type=float, default=100,
                        help='Corridor width in um (default: 100)')
    ...
    # Auto-landmark mode
    auto_group = parser.add_argument_group('auto-landmark mode',
        'Detect vein landmarks from expression patterns')
    auto_group.add_argument('--auto', action='store_true',
                            help='Auto-detect landmarks from marker expression')
    auto_group.add_argument('--cv-marker', default=None,
                            help='Pericentral marker name (e.g., GluI)')
    auto_group.add_argument('--pv-marker', default=None,
                            help='Periportal marker name (e.g., Pck1)')
    auto_group.add_argument('--cluster-radius', type=float, default=100,
                            help='Radius for clustering positive cells (um, default: 100)')
    auto_group.add_argument('--min-cluster-cells', type=int, default=20,
                            help='Min cells per vein cluster (default: 20)')
    auto_group.add_argument('--max-path-length', type=float, default=2000,
                            help='Max CV-PV path length in um (default: 2000)')
    auto_group.add_argument('--all-pairs', action='store_true',
    ...
    if args.auto:
        ...
        if len(cv_centroids) == 0 or len(pv_centroids) == 0:
            logger.error("Need at least 1 CV and 1 PV cluster. "
        ...
        logger.info(f"Zonation scores: mean={scores.mean():.3f}, "
        ...
        for m in markers:
            all_marker_vals[m] = np.array([
                detections[vi].get('features', {}).get(f'{m}_value', 0.0)
                for vi in valid_indices
            ], dtype=np.float64)
        global_data = {'frac_pos': scores}
        global_data.update(all_marker_vals)
        plot_zonation_gradient(global_data, markers, 'all_cells_auto',
                               0, out_dir, args.n_bins)

        # Generate auto paths between nearest CV-PV pairs
        paths = generate_auto_paths(cv_centroids, pv_centroids, args.max_path_length,
                                    all_pairs=args.all_pairs)

        # Spatial zonation map (with paths overlaid)
        plot_zonation_map(positions, scores, cv_centroids, pv_centroids, out_dir,
                          title=f'Auto zonation: {args.cv_marker}(CV) vs {args.pv_marker}(PV)',
                          paths=paths, corridor_um=args.corridor_um)

        # Discrete zone map with labeled veins and routes
        plot_zone_map(positions, scores, cv_centroids, pv_centroids, out_dir,
                      paths=paths)
        ...
        landmarks = {
            ...
            'auto_paths': [{'name': p['name'], 'cv': p['waypoints'][0].tolist(),
                            'pv': p['waypoints'][1].tolist(),
                            'length_um': p['length_um']} for p in paths],
        }
    ...
    for path_info in paths:
        corridor_um = args.corridor_um if args.corridor_um is not None else path_info.get('corridor_um', 100)
        corridor_half = corridor_um / 2.0
        path_length = float(np.sum(np.linalg.norm(np.diff(path_info['waypoints'], axis=0), axis=1)))

        logger.info(f"Path '{path_info['name']}': {len(path_info['waypoints'])} waypoints, "
        ...
        if n_in == 0:
            logger.warning(f"  No cells in corridor — skipping")
            continue

        # Get indices of cells in corridor
        corridor_global_indices = [valid_indices[i] for i in range(len(in_corridor)) if in_corridor[i]]
        ...
        plot_zonation_gradient(path_data, markers, path_info['name'], path_length,
                               out_dir, args.n_bins)
        ...
        for marker in markers:
            ...
            path_summary[f'{marker}_spearman_rho'] = round(float(rho), 4) if np.isfinite(rho) else None
            path_summary[f'{marker}_spearman_pval'] = float(f'{pval:.2e}') if np.isfinite(pval) else None
