...
def load_and_prepare(
    ...
    if marker_channels is None:
        channel_keys = _auto_detect_feature_keys(
            detections, mode=feature_mode, exclude_channels=exclude_channels
        )
    else:
        # Apply exclusion to explicitly provided keys too
        if exclude_channels:
            channel_keys = [
                k for k in marker_channels if not _should_exclude_key(k, exclude_channels)
            ]
        else:
    ...
    if n_pca is not None and n_pca < features.shape[1]:
        ...
        print(
            f"  PCA: {len(channel_keys)} features -> {n_pca} components "
...
def build_spatial_graph(positions, k_neighbors=15):
    ...
    if k < 1:
        return None
    graph = kneighbors_graph(positions, n_neighbors=k, mode="connectivity", include_self=False)
...
def find_zones_elbow(features, connectivity, min_zones=3, max_zones=20, scale=1.0):
    ...
    if n < 3:
        ...
        if line_len < 1e-12:
            ...
            perp_dist = (
                np.abs(dy * x_norm - dx * y_norm + x_norm[-1] * y_norm[0] - y_norm[-1] * x_norm[0])
                / line_len
            )
...
def find_zones(
    ...
    if N > max_direct_cells:
        labels, n_found, elbow_info = _subsample_and_propagate(
            ...
            max_direct_cells=max_direct_cells,
            min_zones=min_zones,
            max_zones=max_zones,
            scale=scale,
        )
...
def auto_label_zones(zone_metadata, channel_keys, channel_names=None, features_raw=None):
...
def gate_label_zones(
    ...
    for j, (key, name) in enumerate(zip(channel_keys, channel_names)):
        ...
        print(
            f"  {name}: threshold = {thresholds[name]:.1f} ({src}), "
    ...
    for zone in zone_metadata:
        ...
        zone["gate_thresholds"] = {name: float(thresholds[name]) for name in channel_names}
...
def gate_cells(
    ...
    for marker in gate_markers:
        ...
        if friendly in gate_thresholds:
            ...
            print(
                f"  {friendly}: p{percentile} threshold = {thresh:.1f} "
            ...
            n_pos = int((features_raw[:, idx] > thresh).sum())
            print(
                f"  {friendly}: Otsu threshold = {thresh:.1f} "
                f"({n_pos}/{len(features_raw)} positive, "
                f"{100*n_pos/len(features_raw):.1f}%)"
            )
...
def find_structures_per_type(
    positions, labels, zone_metadata, eps_um=None, min_cells=10, eps_scale=1.0
):
            ...
            structure_metadata.append(
                {
                    ...
                    "centroid_um": (
                        [float(type_pos[:, 0].mean()), float(type_pos[:, 1].mean())]
                        if len(type_pos) > 0
                        else [0, 0]
                    ),
                }
            ...
            structure_metadata.append(
                {
                    ...
                    "extent_um": [
                        float(np.ptp(cluster_pos[:, 0])),
                        float(np.ptp(cluster_pos[:, 1])),
                    ],
                    "density_cells_per_mm2": float(
                        cluster_mask.sum()
                        / max(np.ptp(cluster_pos[:, 0]) * np.ptp(cluster_pos[:, 1]) * 1e-6, 1e-12)
                    ),
                }
            )
            struct_id_counter += 1
            ...
            structure_metadata.append(
                {
                    ...
                    "centroid_um": [float(noise_pos[:, 0].mean()), float(noise_pos[:, 1].mean())],
                }
            )
            struct_id_counter += 1

        print(
            f"  {type_label}: {n_clusters} structures + "
...
def merge_rare_groups(
    labels, zone_metadata, positions, features_raw, channel_keys, min_group_cells=24
):
    ...
    other_meta = {
        ...
        "centroid_um": (
            [float(other_pos[:, 0].mean()), float(other_pos[:, 1].mean())]
            if other_mask.any()
            else [0, 0]
        ),
        "extent_um": (
            [float(np.ptp(other_pos[:, 0])), float(np.ptp(other_pos[:, 1]))]
            if other_mask.any()
            else [0, 0]
        ),
        "marker_profile": {},
    ...
    new_labels = np.array([old_to_new.get(int(lbl), other_meta["zone_id"]) for lbl in labels])
...
def plot_zone_map(
    positions, labels, zone_metadata, output_path, title="Tissue Zones", color_by_group=False
):
    ...
    for zone in zone_metadata:
        ...
        ax.scatter(
            ...
            s=3,
            alpha=0.6,
            label=f"Z{zid}: {zone['zone_label']} (n={zone['n_cells']})",
        )
    ...
    ax.invert_yaxis()
    ax.legend(
        fontsize=7, loc="upper left", bbox_to_anchor=(1.02, 1), markerscale=3, borderaxespad=0
    )
...
def _plot_zone_map_by_group(positions, labels, zone_metadata, output_path, title):
    ...
    group_cells = dict.fromkeys(groups, 0)
    ...
    for g in groups:
        ...
        n = group_cells[g]
        ax.scatter(
            positions[mask, 0], positions[mask, 1], c=[color], s=3, alpha=0.6, label=f"{g} (n={n})"
        )
    ...
    ax.invert_yaxis()
    ax.legend(
        fontsize=8, loc="upper left", bbox_to_anchor=(1.02, 1), markerscale=3, borderaxespad=0
    )
...
def plot_zone_map_with_density(
    positions,
    labels,
    zone_metadata,
    output_path,
...
def plot_gate_histograms(
    features_raw, channel_keys, channel_names, gate_markers, thresholds, output_path
):
    ...
    for ax, idx, name in zip(axes, gate_indices, gate_names):
        ...
        ax.axvline(
            thresh, color="red", linewidth=2, linestyle="--", label=f"threshold = {thresh:.0f}"
        )
...
def plot_kdistance(kdist_data, eps_used, output_path):
    ...
    for ax, (label, (k_dists, elbow_eps)) in zip(axes, kdist_data.items()):
        ...
        # Mark auto elbow
        elbow_idx = np.searchsorted(
            -k_dists, -elbow_eps
        )  # find index where k_dists drops below elbow_eps
...
def plot_dendrogram_elbow(elbow_info, chosen_k, output_path):
    ...
    ax.axvline(natural_k, color="green", linestyle="--", alpha=0.7, label=f"elbow k={natural_k}")
    if chosen_k != natural_k:
        ax.axvline(chosen_k, color="red", linestyle="--", alpha=0.7, label=f"final k={chosen_k}")
...
def main():
    ...
    parser.add_argument(
        "--detections", required=True, help="Path to detection JSON from run_segmentation.py"
    )
    parser.add_argument("--output-dir", required=True, help="Output directory for zoned results")
    ...
    parser.add_argument(
        "--min-zones", type=int, default=3, help="Min zones for auto search (default: 3)"
    )
    parser.add_argument(
        "--max-zones", type=int, default=20, help="Max zones for auto search (default: 20)"
    )
    parser.add_argument(
        "--max-direct-cells",
        type=int,
    ...
    parser.add_argument(
        "--k-neighbors", type=int, default=15, help="k-NN spatial graph connectivity (default: 15)"
    )
    parser.add_argument(
        "--min-cells-per-zone",
        type=int,
    ...
    # Visualization
    parser.add_argument(
        "--visualize", action="store_true", help="Open napari viewer after zone assignment"
    )
    ...
    ch_mean_keys = _auto_detect_feature_keys(
        detections, mode="channels", exclude_channels=exclude_channels
    )
    if not ch_mean_keys:
    ...
    if gate_markers:
        ...
        for z in zone_metadata:
            ...
            z["centroid_um"] = [float(zone_pos[:, 0].mean()), float(zone_pos[:, 1].mean())]
            z["extent_um"] = [float(np.ptp(zone_pos[:, 0])), float(np.ptp(zone_pos[:, 1]))]
        n_zones = len(zone_metadata)
    else:
        ...
        if args.label_by_gate:
            ...
            auto_label_zones(zone_metadata, char_keys, char_names, features_raw=features_raw)
            thresholds = None
    ...
    for z in zone_metadata:
        print(
            f"  Zone {z['zone_id']}: {z['zone_label']:30s} "
    ...
    if gate_markers and args.find_structures:
        ...
        for z in zone_metadata:
            ...
            if "centroid_um" not in z:
                z["centroid_um"] = [float(z_pos[:, 0].mean()), float(z_pos[:, 1].mean())]
            if "extent_um" not in z:
                z["extent_um"] = [float(np.ptp(z_pos[:, 0])), float(np.ptp(z_pos[:, 1]))]
    ...
    assign_and_save(detections, labels, zone_metadata, output_dir / "detections_zoned.json")
    ...
    plot_zone_map(
        ...
        labels,
        zone_metadata,
        output_dir / "zone_map.png",
        title=title,
        color_by_group=is_structures,
    )

    plot_marker_profiles(
        zone_metadata, char_keys, char_names, output_dir / "zone_marker_profiles.png"
    )

    plot_zone_map_with_density(
        positions,
        labels,
        zone_metadata,
    ...
    if elbow_info is not None:
        plot_dendrogram_elbow(elbow_info, n_zones_pre_merge, output_dir / "dendrogram_elbow.png")
    ...
    if kdist_data:
        plot_kdistance(kdist_data, args.structure_eps, output_dir / "kdistance_elbow.png")
    ...
    print("  zone_map.png              — cells colored by zone")
    print("  zone_marker_profiles.png  — marker expression per zone")
    print("  zone_map_with_density.png — zones over density heatmap")
    if elbow_info is not None:
        print("  dendrogram_elbow.png      — auto k-selection (elbow method)")
