...
def build_density_profile(distances, transect_length, bin_width_um, weights=None):
    ...
    try:
        ...
        hist, edges = np.histogram(
            distances, bins=n_bins, range=(0, transect_length), weights=weights
        )
        # bin centers
...
def detect_boundaries(scalogram, positions_um, min_prominence_frac=0.1, min_distance_um=50):
...
def plot_scalogram(
    scalogram, positions_um, channel_name, boundaries=None, density=None, output_path=None
):
    """Plot CWT scalogram with optional boundary lines and density profile."""
    fig, axes = plt.subplots(
        3, 1, figsize=(14, 10), sharex=True, gridspec_kw={"height_ratios": [1, 2, 1]}
    )
...
def main():
    ...
    parser.add_argument(
        "--n-transects", type=int, default=5, help="Number of auto-generated transects (default: 5)"
    )
    parser.add_argument(
        "--corridor-width-um",
        type=float,
    ...
    for t_idx, transect in enumerate(transects):
        ...
        print(
            f"  {len(projection['distances'])} cells in corridor "
    ...
    plot_2d_heatmap(hm_data, "cell_density", output_path=output_dir / "heatmap_2d_cell_density.png")
    ...
    for ch_key in ch_keys:
        ...
        plot_2d_heatmap(hm_data, ch_label, output_path=output_dir / f"heatmap_2d_{ch_label}.png")
    ...
    print(
        f"  Total boundaries detected: {sum(len(b['positions_um']) for b in all_boundaries.values())}"
    )
