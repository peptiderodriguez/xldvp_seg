...
def parse_args():
    ...
    parser.add_argument(
        "--tiles-dir", type=str, default=None, help="Tiles directory (default: <run-dir>/tiles)"
    )
    parser.add_argument(
        "--cell-type",
        type=str,
    ...
    parser.add_argument(
        "--pixel-size", type=float, required=True, help="Pixel size in um/px (from CZI metadata)"
    )
...
def main():
    ...
    print(
        f"  Dedup: {pre_dedup} -> {len(all_detections)} "
    ...
    with open(csv_file, "w") as f:
        ...
        for det in all_detections:
            ...
            f.write(
                f"{det.get('uid','')},{g_center[0]:.1f},{g_center[1]:.1f},"
    ...
    print("To regenerate HTML, re-run the full pipeline or use a crop-from-disk script.")
